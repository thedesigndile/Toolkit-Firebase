...
export function SplitPdfOptions({ options, setOptions }: SplitPdfOptionsProps) {
      ...
      <Tabs value={options.mode} onValueChange={(value) => setOptions({ ...options, mode: value as 'ranges' | 'extract' })} className="w-full">
        <TabsList className="grid w-full grid-cols-2">
          <TabsTrigger value="ranges">
            <SlidersHorizontal className="mr-2 h-4 w-4 text-primary" /> Split by range
          </TabsTrigger>
          <TabsTrigger value="extract">
            <FileText className="mr-2 h-4 w-4 text-primary" /> Extract pages
          </TabsTrigger>
        </TabsList>
